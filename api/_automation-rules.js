...
const { buildSequenceEmail, createAndSendEmail } = require("./_automation-email");
...
async function getLead(supabase, leadId) {
  const { data, error } = await supabase
    .from("leads")
    .select("id, athlete_first_name, athlete_last_name, parent_email, readiness_check_url, current_state, funnel_stage, lead_score, first_email_sent, first_email_sent_at, readiness_started_at, readiness_completed_at")
    .eq("id", leadId)
    .single();
...
async function sendSequenceEmail({ supabase, lead, sequenceKey, req, queueItem }) {
  ...
  const result = await createAndSendEmail({
    ...
    req,
    metadata: {
      automationQueueId: queueItem.id,
      ruleKey: queueItem.rule_key,
      sequenceKey
    }
  });
...
async function completeAutomation(supabase, queueItem, result) {
  ...
  await supabase.from("automation_history").insert({
    automation_queue_id: queueItem.id,
    lead_id: queueItem.lead_id,
    rule_key: queueItem.rule_key,
    action_type: result?.actionType || "rule_completed",
...
async function failAutomation(supabase, queueItem, error) {
  ...
  await supabase
    ...
    .update({
      ...
      attempts,
      run_after: exhausted ? queueItem.run_after : minutesFromNow(Math.min(60, 5 * attempts)),
      failed_at: exhausted ? new Date().toISOString() : null,
      error_message: String(error?.message || error || "Unknown automation error").slice(0, 2000),
      locked_at: null,
      locked_by: null
    })
    .eq("id", queueItem.id);
...
async function handleFirstEmailUnopened24h({ supabase, queueItem, req }) {
  ...
  if (lead.current_state === "form_completed" || lead.readiness_completed_at) {
    return { skipped: true, reason: "form_completed", actionType: "no_email_needed" };
  }
  ...
  if (opened) {
    await enqueueAutomation(supabase, {
      leadId: lead.id,
      ruleKey: "opened_no_click_24h",
      runAfter: minutesFromNow(1),
      priority: 60,
      payload: { sourceRule: queueItem.rule_key },
      dedupeKey: `${lead.id}:opened_no_click_24h`
    });
  ...
  const result = await sendSequenceEmail({
    supabase,
    lead,
    sequenceKey: "first_email_unopened_24h",
    req,
    queueItem
  });
  ...
  await enqueueAutomation(supabase, {
    leadId: lead.id,
    ruleKey: "first_email_unopened_72h",
    runAfter: addHours(new Date(), 48),
    priority: 100,
    payload: { sourceRule: queueItem.rule_key },
    dedupeKey: `${lead.id}:first_email_unopened_72h`
  });
...
async function handleOpenedNoClick24h({ supabase, queueItem, req }) {
  ...
  const clicked = await hasEvent(supabase, lead.id, "EMAIL_CLICKED");
  if (clicked) {
    return { skipped: true, reason: "already_clicked", actionType: "no_email_needed" };
  }

  const result = await sendSequenceEmail({
    supabase,
    lead,
    sequenceKey: "opened_no_click_24h",
    req,
    queueItem
  });
...
async function handleFirstEmailUnopened72h({ supabase, queueItem, req }) {
  ...
  const opened = await hasEvent(supabase, lead.id, "EMAIL_OPENED");
  if (opened) {
    return { skipped: true, reason: "email_opened", actionType: "no_email_needed" };
  }
...
async function handleReadinessFollowup24h({ supabase, queueItem, req }) {
  ...
  if (!lead.readiness_completed_at && lead.current_state !== "form_completed") {
    return { skipped: true, reason: "form_not_completed", actionType: "no_email_needed" };
  }

  const result = await sendSequenceEmail({
    supabase,
    lead,
    sequenceKey: "readiness_followup_24h",
    req,
    queueItem
  });
...
async function handleReviewCompletedReadinessCheck({ supabase, queueItem }) {
  ...
  if (!lead.readiness_completed_at && lead.current_state !== "form_completed") {
    return { skipped: true, reason: "form_not_completed", actionType: "no_review_needed" };
  }

  await recordLeadEvent(supabase, {
    leadId: lead.id,
    eventType: "READINESS_REVIEW_QUEUED",
    source: "automation",
    metadata: {
      automationQueueId: queueItem.id,
      readinessCheckId: queueItem.payload?.readinessCheckId || null
    },
    idempotencyKey: `readiness-review-queued:${queueItem.id}`
  });
...
async function handleGenerateFirstEmailContext({ supabase, queueItem }) {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      first_email_context_ready_at: new Date().toISOString(),
      first_email_question_context: "That is the right question to ask. Recruiting gets easier when the athlete has a clear starting point, a realistic board, and a weekly plan to follow.",
      biggest_question_theme: "recruiting_plan",
  ...
  await enqueueAutomation(supabase, {
    ...
    priority: 20,
    payload: { sourceRule: queueItem.rule_key },
    dedupeKey: `${lead.id}:send_first_email`
  });
...
async function handleSendFirstEmail({ supabase, queueItem }) {
  // Existing /api/send-ready-lead-emails remains the sender of record for the first email.
  // This rule marks queue progress while leaving first-email delivery to the established daily sender.
  const lead = await getLead(supabase, queueItem.lead_id);

  if (lead.first_email_sent || lead.first_email_sent_at) {
    return { skipped: true, reason: "already_sent", actionType: "no_email_needed" };
  }
...
async function runAutomationRule({ supabase, queueItem, req }) {
  ...
  if (!handler) {
    return { skipped: true, reason: "unknown_rule", actionType: "unknown_rule" };
  }
