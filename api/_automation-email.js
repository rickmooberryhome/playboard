                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin-top:24px;">
                  <tr>
                    <td style="border-radius:999px; background:#f7e913;">
                      <a href="${actionUrl || "{{ACTION_URL}}"}" style="display:inline-block; padding:15px 22px; color:#080808; font-size:13px; line-height:16px; font-weight:900; text-decoration:none; text-transform:uppercase; letter-spacing:0.08em; border-radius:999px;">${escapeHtml(actionLabel)}</a>
                    </td>
                  </tr>
                </table>
...
function buildSequenceEmail({ sequenceKey, lead }) {
  ...
  const emails = {
    ...
    readiness_abandoned_30m: {
      ...
      html: buildBaseTemplate({
        eyebrow: "Finish the Check",
        headline: "Not sure is okay.",
        bodyHtml: `<p style="margin:0 0 14px 0; color:#b5bec8; font-size:16px; line-height:25px;">Looks like you started the Recruiting Readiness Check.</p><p style="margin:0; color:#b5bec8; font-size:16px; line-height:25px;">You do not need perfect answers. If something is missing, write <strong style="color:#ffffff;">Not sure</strong> and keep moving. The goal is to understand the starting point.</p>`,
    ...
    readiness_followup_24h: {
      ...
      html: buildBaseTemplate({
        ...
        bodyHtml: `<p style="margin:0 0 14px 0; color:#b5bec8; font-size:16px; line-height:25px;">Thanks for completing the Recruiting Readiness Check.</p><p style="margin:0; color:#b5bec8; font-size:16px; line-height:25px;">The next step is reviewing the starting point and turning it into a clear plan. We will look at fit, film, academics, outreach, and what should happen next.</p>`,
        actionLabel: "Back to PlayBoard",
        fallbackText: "You received this because you completed the PlayBoard readiness check.",
        actionUrl: configuredSiteUrl || "https://playboardfootball.com"
      }),
...
async function createAndSendEmail({ supabase, lead, email, req, metadata = {} }) {
  ...
  const { data: message, error: insertError } = await supabase
    ...
    .insert({
      ...
      provider: dryRun ? "dry_run" : "resend",
  ...
  const html = addTrackingToHtml({
    html: email.html,
    baseUrl,
    leadId: lead.id,
    messageId: message.id,
    targetUrl: email.targetUrl
  });
